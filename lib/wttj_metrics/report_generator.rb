...
module WttjMetrics
  ...
  class ReportGenerator
    ...
    def initialize(csv_path, days: 90, teams: nil)
      ...
      @parser = MetricsParser.new(csv_path)
      @data = @parser.data
    ...
    def cycles_parsed
      @cycles_parsed ||= CycleParser.new(@parser.metrics_for('cycle', date: nil), teams: @selected_teams).parse
    end

    def cycles_by_team
      @cycles_by_team ||= CycleParser.new(@metrics_by_category['cycle'], teams: @selected_teams).by_team
    end
